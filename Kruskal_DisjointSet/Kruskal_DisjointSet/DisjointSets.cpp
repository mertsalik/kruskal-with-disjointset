...
DisjointSets::DisjointSets(const DisjointSets & s)
{
	...
	for(int i = 0; i < m_numElements; ++i)
		m_nodes[i] = s.m_nodes[i];// new Node(*s.m_nodes[i]);
...
int DisjointSets::FindSet(int elementId) const
{
	...
	while(curNode != root)
	{
		DisjointSetItem* next = curNode->parent;
		curNode->parent = root;
		curNode = next;
	}
...
void DisjointSets::Union(int setId1, int setId2)
{
	...
	DisjointSetItem* set1 = m_nodes[setId1];
	DisjointSetItem* set2 = m_nodes[setId2];
...
void DisjointSets::AddElement(DisjointSetItem* setItem){
	int numToAdd = 1;
	m_nodes.insert(m_nodes.end(),numToAdd,setItem);
